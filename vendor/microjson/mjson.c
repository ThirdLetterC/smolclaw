...
static bool json_match_json_number(const char *s, bool *is_integer)
{
	...
	if (is_integer != NULL) {
		*is_integer = !(saw_fraction || saw_exponent);
	}
	return true;
}

static bool json_parse_long_strict(const char *s, long *out)
{
	char *ep = NULL;
	long v;
	bool is_integer;

	if (!json_match_json_number(s, &is_integer) || !is_integer) {
		return false;
	}
	errno = 0;
	v = strtol(s, &ep, 10);
	if (errno == ERANGE || ep == s || *ep != '\0') {
		return false;
	}
	*out = v;
	return true;
...
static bool json_parse_ulong_strict(const char *s, unsigned long *out)
{
	char *ep = NULL;
	unsigned long v;
	bool is_integer;

	if (!json_match_json_number(s, &is_integer) || !is_integer || *s == '-') {
		return false;
	}
	errno = 0;
	v = strtoul(s, &ep, 10);
...
static bool json_parse_double_strict(const char *s, double *out)
{
	char *ep = NULL;
	double v;

	if (!json_match_json_number(s, NULL)) {
		return false;
	}
	errno = 0;
	v = strtod(s, &ep);
...
{
	bool is_integer;
	return json_match_json_number(s, &is_integer) && is_integer;
}

static bool json_number_is_real(const char *s)
{
	bool is_integer;
	return json_match_json_number(s, &is_integer) && !is_integer;
}
	...
	for (; *cp != '\0'; cp++) {
		...
		case await_value:
			if (isspace((unsigned char)*cp) || *cp == ':') {
				...
				--cp; /* json_read_array() returns first unconsumed char */
				state = post_element;
		...
		case post_val:
		  ...
		  if (isspace((unsigned char)*cp)) {
			...
			for (;;) {
				...
				if (value_quoted &&
				    (seeking == t_string || seeking == t_time)) {
					break;
				}
				...
				if (!value_quoted) {
					...
					if (json_number_is_integer(valbuf) &&
					    (seeking == t_integer ||
					     seeking == t_uinteger ||
					     seeking == t_short ||
					     seeking == t_ushort)) {
					...
					if (json_number_is_real(valbuf) && seeking == t_real) {
						break;
					}
				}

				if (cursor[1].attribute ==
				    NULL) /* out of possibilities */
					break;
			...
			if (lptr != NULL) {
				...
				case t_integer: {
					...
					if (!json_parse_long_strict(valbuf, &tmp) ||
					    tmp < INT_MIN || tmp > INT_MAX) {
						return JSON_ERR_BADNUM;
					}
					narrowed = (int)tmp;
					memcpy(lptr, &narrowed, sizeof(int));
				} break;
				case t_uinteger: {
					unsigned long tmp;
					unsigned int narrowed;

					if (!json_parse_ulong_strict(valbuf, &tmp) ||
					    tmp > UINT_MAX) {
						return JSON_ERR_BADNUM;
					}
					narrowed = (unsigned int)tmp;
					memcpy(lptr, &narrowed, sizeof(unsigned int));
				} break;
				case t_short: {
					long tmp;
					short narrowed;
					...
					narrowed = (short)tmp;
					memcpy(lptr, &narrowed, sizeof(short));
				} break;
				case t_ushort: {
					unsigned long tmp;
					...
					narrowed = (unsigned short)tmp;
					memcpy(lptr, &narrowed, sizeof(unsigned short));
				} break;
				case t_time:
#ifdef TIME_ENABLE
				{
				...
				case t_real: {
					...
					if (!json_parse_double_strict(valbuf, &tmp)) {
						return JSON_ERR_BADNUM;
					}
					memcpy(lptr, &tmp, sizeof(double));
				} break;
